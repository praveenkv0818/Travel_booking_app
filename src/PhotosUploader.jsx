...
export default function PhotosUploader({addedPhotos,onChange}) {
  ...
  async function addPhotoByLink(ev) {
    ...
    try {
      const { data: { imageUrl } } = await axios.post('/upload-by-link', { link: photoLink });
      
      onChange(prev => [...prev, imageUrl]); // Update state with the new image URL
      
      setPhotoLink(''); // Clear input after submission
    } catch (error) {
  ...
  function uploadPhoto(ev) {
    ...
    data.append('image', files[0]); // 'image' for single file upload
  
    axios.post('/upload', data, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    .then(response => {
      console.log("Response after making request to backend: ", response);
      const { data: filename } = response;
      onChange(prev => [...prev, filename.imageUrl]);
    })
    .catch(error => {
